...
static void ip_dump(const uint8_t *data, size_t len) {
    ...
    // IP headerのデータ構造と同じ構造体なのでキャストできる
...
int ip_iface_register(struct net_device *dev, struct ip_iface *iface) {
    ...
    // routing table には network address で登録する
...
static void ip_input(const uint8_t *data, size_t len, struct net_device *dev) {
    ...
    hlen = (hdr->vhl & 0x0f) << 2;
    if (len < hlen) {
        errorf("invalid hlen: %u", hlen);
        return;
    }
    total = ntoh16(hdr->total);
    if (len < total) {
        errorf("invalid total len: %u", total);
        return;
    }
    ...
    if (NET_IFACE(iface)->dev->flags & NET_DEVICE_FLAG_NEED_ARP) {
        if (dst == iface->broadcast || dst == IP_ADDR_BROADCAST) {
            ...
            if (ret != ARP_RESOLVE_FOUND) {
                infof("arp_resolve() failure, ret=%d", ret);
                return ret;
            }
        }
    }
...
ssize_t ip_output(uint8_t protocol, const uint8_t *data, size_t len,
    ...
    if (src != IP_ADDR_ANY && src != iface->unicast) {
        errorf("unable to output with specified source address, addr=%s, iface=%s",
               ip_addr_ntop(src, addr, sizeof(addr)),
               ip_addr_ntop(iface->unicast, addr2, sizeof(addr)));
        return -1;
    }
