...
static int arp_request(struct net_iface *iface, ip_addr_t tpa) {
    ...
    return net_device_output(iface->dev, ETHER_TYPE_ARP, (uint8_t *)&request,
                             sizeof(request), ETHER_ADDR_BROADCAST);
}
...
static void arp_input(const uint8_t *data, size_t len, struct net_device *dev) {
    ...
    if (ntoh16(msg->hdr.hrd) != ARP_HRD_ETHER) {
        errorf("unsupported hardware type: 0x%04x", ntoh16(msg->hdr.hrd));
        return;
    }
    if (ntoh16(msg->hdr.pro) != ARP_PRO_IP) {
        errorf("unsupported protocol type: 0x%04x", ntoh16(msg->hdr.pro));
        return;
    }
    ...
    if (iface && ((struct ip_iface *)iface)->unicast == tpa) {
        ...
        } else if (ntoh16(msg->hdr.op) == ARP_OP_REPLY) {
            debugf("ARP reply");
        }
    } else {
        debugf("SPA != unicast address");
...
int arp_resolve(struct net_iface *iface, ip_addr_t pa, uint8_t *ha) {
    ...
    if (!cache) {
        ...
        cache = arp_cache_insert(pa, ETHER_ADDR_ANY);
        if (!cache) {
            errorf("arp_cache_insert() failure");
            mutex_unlock(&mutex);
            return ARP_RESOLVE_ERROR;
        }
...
static void arp_timer_handler(void) {
    ...
    for (entry = caches; entry < tailof(caches); entry++) {
        if (entry->state != ARP_CACHE_STATE_FREE &&
            ...
            struct timeval timeout = {ARP_CACHE_TIMEOUT, 0};
